		...
		if (dia.isAfter(userRequest.getDataVencimento()) || dia.isBefore(userRequest.getDataPodeIniciarFerias())) {
			...
			if (userRequest.getNivel() == Nivel.SENIOR) {
				if (senior >= 1 && quantMembros <= 12) {
					...
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Sêniors de folga para um time médio já foi alcançado (2)!",
							df.format(dia));
					return (response);
				} else if (senior >= 5 && quantMembros > 24) {
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Sêniors de folga para um time grande já foi alcançado (5)!",
							df.format(dia));
					return (response);
				} else {
					return "OK";
				...
				if (pleno >= 1 && quantMembros <= 12) {
					...
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Plenos de folga para um time médio já foi alcançado (3)!",
							df.format(dia));
					return (response);
				} else if (pleno >= 5 && quantMembros > 24) {
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Plenos de folga para um time grande já foi alcançado (5)!",
							df.format(dia));
					return (response);
				} else {
					return "OK";
				...
				if (junior >= 1 && quantMembros <= 12) {
					...
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Juniors de folga para um time médio já foi alcançado (3)!",
							df.format(dia));
					return (response);
				} else if (junior >= 5 && quantMembros > 24) {
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Juniors de folga para um time grande já foi alcançado (5)!",
							df.format(dia));
					return (response);
				} else {
					return "OK";
				...
				if (trainee >= 1 && quantMembros <= 12) {
					...
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Treiness de folga para um time médio já foi alcançado (3)!",
							df.format(dia));
					return (response);
				} else if (trainee >= 5 && quantMembros > 24) {
					response = String.format(
							"ERROR! Não foi possível marcar as férias no dia %s! Número máximo de Treinees de folga para um time grande já foi alcançado (5)!",
							df.format(dia));
					return (response);
				} else {
					return "OK";
